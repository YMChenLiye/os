...
bool search_dir_entry(struct partition* part,struct dir* pdir,const char* name,struct dir_entry* dir_e){
	...
	while(block_idx < block_cnt){
		...
		block_idx++;
		p_de = (struct dir_entry*)buf;		//此时p_de已经指向扇区内最后有一个完整目录项了，需要恢复p_de指向buf
...
void dir_close(struct dir* dir){
	//------------ 根目录不能关闭 --------------------
	//1 根目录自打开后就不应该关闭，否则还需要再此open_root_dir();
	...
	while(block_idx < 140){	//文件(包括目录)最大支持12个直接块 + 128个间接块 = 140个块
		...
		if(all_blocks[block_idx] == 0){		//在三种情况下分配块
			...
			if(block_idx < 12){		//若是直接块
				...
				dir_inode->i_sectors[12] = block_lba;	//将上面分配的块作为一级间接块地址
				block_lba = -1;
...
bool delete_dir_entry(struct partition* part,struct dir* pdir,uint32_t inode_no,void* io_buf){
	...
	//目录项在存储时保证不会跨扇区
	uint32_t dir_entry_size = part->sb->dir_entry_size;
	uint32_t dir_entrys_per_sec = (SECTOR_SIZE / dir_entry_size);	//每扇区最大的目录项数
	...
	while(block_idx < 140){
		...
		while(dir_entry_idx < dir_entrys_per_sec){
			if((dir_e + dir_entry_idx)->f_type != FT_UNKNOWN){
				if(!strcmp((dir_e + dir_entry_idx)->filename,".")){
					...
					dir_entry_cnt++;	//统计此扇区内的目录个数，用来判断删除目录项后是否该回收该扇区
		...
		//除目录第一个扇区外，若该扇区上只有该目录项自己，则将整个扇区回收
		if(dir_entry_cnt == 1 && !is_dir_first_block){
			...
			if(block_idx < 12){
				...
				//先判断一级间接索引表中间接块的数量，如果仅有这一个间接块，连同间接索引表所在的块一同回收
				...
				if(indirect_blocks > 1){	//间接索引表中还包括其他间接块，仅在索引表中擦除当前这个简介块
					all_blocks[block_idx] = 0;
					ide_write(part->my_disk,dir_inode->i_sectors[12],all_blocks + 12,1);
				}else{		//间接索引表中就当前这一个间接块，直接把间接索引表中所在的块回收，然后擦除间接索引表块地址
					//回收间接索引表所在的块
	...
	//所有块中未找到则返回false，若出现这种情况应该是search_file出错了
	return false;
}
