...
//在分区链表中找到名为part_name的分区，并将其指针赋值为cur_part
...
static void partition_format(struct partition* part){
	...
	// 1 先将位图最后一字节到其所在的扇区的结束全置1，即超出实际块数的部分直接置为已占用
	...
	//由于inode_table中共有4096个inode，位图inode_bitmap正好占用1扇区
	...
	i->i_no = 0;							//根目录占据inode数组中的第0个inode
...
int32_t sys_open(const char* pathname,uint8_t flags){
	...
	//先判断是否把pathname的各层目录都访问到，即是否在某个中间目录就失败了
	...
	//若是在最后一个路径上没有找到，并且并不是要创建文件，直接返回-1
	...
	if(inode_no == -1){
		...
		if(searched_record.file_type == FT_REGULAR){
			...
			if(!dir_is_empty(dir)){	//非空目录不可删除
				printk("dir %s is not empty,it is not allowed to delete a noempty directory!\n",pathname);
			}else{
...
static uint32_t get_parent_dir_inode_nr(uint32_t child_inode_nr,void* io_buf){
	...
	//目录中的目录项".."中包括父目录inode编号,".."位于目录的第1块
...
//在inode编号为p_indoe_nr的目录中查找inode编号为c_inode_nr的子目录的名字
		...
		while(dev_no < 2){
			...
			while(part_idx < 12){	//4个主分区，8个逻辑分区
				...
				if(part->sec_cnt != 0){		//如果分区存在
					...
					//读出分区的超级块，根据魔数是否正确来判断文件系统
					ide_read(hd,part->start_lba + 1,sb_buf,1);

					//只支持自己的文件系统，若磁盘上已经有文件系统就不再格式化了
					if(sb_buf->magic == 0x19950910){
