...
#define DEFAULT_SECTS  1

//文件表
struct file file_table[MAX_FILE_OPEN];

//文件表file_table中获取一个空闲位，成功返回下标，失败返回-1
...
int32_t file_create(struct dir* parent_dir,char* filename,uint8_t flag){
	...
	create_dir_entry(filename,inode_no,FT_REGULAR,&new_dir_entry);	//create_dir_entry只是内存操作，不出意外，不会返回失败
...
//打开编号inode_no的inode对应的文件，若成功则返回文件描述符，否则返回-1
int32_t file_open(uint32_t inode_no,uint8_t flag){
	...
	file_table[fd_idx].fd_pos = 0;	//每次打开文件，要将fd_pos还原为0，即让文件内的指针指向抬头
	file_table[fd_idx].fd_flag = flag;
	bool* write_deny = &file_table[fd_idx].fd_inode->write_deny;

	if(flag & O_WRONLY || flag & O_RDWR){	//只要是关于写文件，判断是否有其他进程正写此文件，若是读文件，不考虑write_deny
		//以下进入临界区先关中断
		enum intr_status old_status = intr_disable();
		if(!(*write_deny)){		//若当前没有其他进程同时写此文件,将其占用
...
int32_t file_write(struct file* file,const void* buf,uint32_t count){
	...
	uint32_t sec_off_bytes;			//扇区字节偏移量
	...
	if(add_blocks == 0){
		...
		if(file_will_use_blocks <= 12){
			...
			while(block_idx < file_will_use_blocks){
				...
				//写文件时，不应该存在未使用但已经分配扇区的情况，当文件删除时，就会把块地址清0
			...
			ide_read(cur_part->my_disk,indirect_block_table,all_blocks + 12,1);	//获取所有间接块
