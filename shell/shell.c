...
void print_prompt(void){
	printf("[cly@localhost %s]$ ",cwd_cache);
}
...
static int32_t cmd_parse(char* cmd_str,char** argv,char token){
	...
	while(*next){
		...
		if(*next){
			*next++ = 0;	//将token字符替换成字符串结束符0，作为一个单词的结束符，并将字符指针next指向下一个字符
		}
...
void my_shell(void){
	...
	while(1){
		...
		if(argc == -1){
			printf("num of arguments excedd %d\n",MAX_ARG_NR);
			continue;
		}
		if (!strcmp("ls", argv[0])) {
			...
			int32_t pid = fork();
			if(pid){		//父进程
				//下面这个while必须加上，否则父进程一般情况下会比子进程先执行，
				//因此会进行下一轮循环将final_path清空，这样子进程将无法从final_path中获得参数
				int32_t status;
				int32_t child_pid = wait(&status);	//此时子进程没有执行exit，my_shell会被阻塞，不再响应键入的命令
				if(child_pid == -1){	//按理说程序正确的话不会执行到这句，fork出的进程就是shell子进程
