...
//0xc0000000是内核从虚拟地址3G起，0x100000意指跨过底端1M内存，是虚拟地址在逻辑上连续
...
static void page_table_add(void *_vaddr,void *_page_phyaddr){
	...
	if(*pde & 0x00000001){	//目录项和页表项的第0位为P，此处判断目录项是否存在
		ASSERT(!(*pte & 0x00000001));

		if(!(*pte & 0x00000001)){		//只要是创建页表，pte就不应该存在，多判断以下放心
			*pte = (page_phyaddr | PG_US_U | PG_RW_W | PG_P_1);
		}else{		//应该不会执行到这，ASSERT会先执行
			PANIC("pte repeat");
			*pte = (page_phyaddr | PG_US_U | PG_RW_W | PG_P_1);
		}
...
void * malloc_page(enum pool_flags pf,uint32_t pg_cnt){
	...
	//因为虚拟地址是连续的，但物理地址可以是不连续的，所以逐个映射
	while(cnt-- >0){
		void *page_phyaddr = palloc(mem_pool);
		if(page_phyaddr == NULL){
			return NULL;
			//失败时要将曾经已申请的虚拟地址和物理页面都全部回滚，在将来完成内存回收时在补充
		}
...
//从内核物理池中申请pg_cnt页内存，成功则返回其虚拟地址，失败则返回NULL
...
static void mem_pool_init(uint32_t all_mem){
	put_str("    mem_pool_inti start\n");
	uint32_t page_table_size = PG_SIZE * 256;	//页表大小=1页的页目录表+第0和第768个页目录项指向同一个页表+第769～1022个页目录项共指向254个页表，共256个页框
	uint32_t used_mem = page_table_size + 0x100000;	//0x100000为低端1M内存
	uint32_t free_mem = all_mem - used_mem;				//我们的内存管理只管理空闲的内存
	uint16_t all_free_pages = free_mem / PG_SIZE;	//1页为4k，不管总内存是不是4k的倍数，对于以页为单位的内存分配策略，不足一页的内存不用考虑了
	uint16_t kernel_free_pages = all_free_pages / 2;
	uint16_t user_free_pages = all_free_pages - kernel_free_pages;

	//为了简化位图操作，余数不处理，坏处是这样会丢内存
	...
	//	32M内存占用的位图是2k。内核内存池的位图先定在MEN_BITMAP_BASE(0xc009a000)处
...
void* sys_malloc(uint32_t size){
	...
	//超过最大内存块1024，就分配页框
	if(size > 1024){
		uint32_t page_cnt = DIV_ROUND_UP(size + sizeof(struct arena),PG_SIZE);	//向上区整
		...
		//从内存块描述符中匹配合适的内存块规格
		for(desc_idx = 0;desc_idx < DESC_CNT;desc_idx++){
			if(size <= descs[desc_idx].block_size){	//从小往大找，找到后退出
				break;
			}
		}
...
void sys_free(void* ptr){
	...
	if(ptr != NULL){
		...
		if(a->desc == NULL && a->large == true){	//大于1024的内存
			...
			//再判断此arena中的内存块是否都是空闲的，如果是就释放arena
