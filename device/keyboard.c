...
//定义以下变量记录相应键是否按下的状态
// ext_scancode用于记录makecode 是否以0xe0开头
...
static void intr_keyboard_handler(void){
	...
	if(break_code){
		...
		uint16_t make_code = (scancode &= 0xff7f);	//得到其make_code(按键按下时产生的)
	...
	//若为通码，只处理数组中定义的键以及alt_right和ctrl键，全是make_code
		...
		bool shift = false;		//判断是否与shift组合，用来在一堆数组中索引对应的字符
		if( (scancode < 0x0e) || (scancode == 0x29) || \
			...
			if(shift_down_last && caps_lock_last){ 	//如果shift和capslock同时按下
				shift = false;
			}else if(shift_down_last || caps_lock_last){  //有一个被按下
				shift = true;
			}else{
				shift = false;
			}
		...
		uint8_t index = (scancode &= 0x00ff);	//将扫描码的高字节置0，主要是针对高字节是e0
